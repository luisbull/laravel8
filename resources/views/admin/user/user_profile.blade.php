                ...
                <form action="{{ route('profile.update.dashboard') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="profile_name" class="form-control" id="profileName"
                            placeholder="Profile name" value="{{ $userProfile->name }}">
                        @error('profile_name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="profile_email" class="form-control" id="profileEmail"
                            placeholder="Profile email" value="{{ $userProfile->email }}">
                        @error('profile_email')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <input type="file" name="profile_image" class="form-control" id="profileImage"
                            value="{{ asset('storage/' . $userProfile->profile_photo_path) }}">
                        @error('profile_image')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="">
                        <img src="{{ asset('storage/' . $userProfile->profile_photo_path) }}" class="w-25 p-3"
                            alt="" srcset="">
                    </div>
